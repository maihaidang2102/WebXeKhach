...
/**
 * Servlet implementation class SortTripServlet
 */
@WebServlet(urlPatterns = { "/sortTrip" })
public class SortTripServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public SortTripServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// get list trip according to request sort from interface
		if (sortInt == 1) {
			list_trip = TripDAO.getListTripOrderByTimeIncre(conn, departure, destination, date);
		}
		if (sortInt == 2) {
			list_trip = TripDAO.getListTripOrderByTimeDecre(conn, departure, destination, date);
		}
		if (sortInt == 3) {
			list_trip = TripDAO.getListTripOrderByPriceIncre(conn, departure, destination, date);
		}
		if (sortInt == 4) {
			list_trip = TripDAO.getListTripOrderByPriceDecre(conn, departure, destination, date);
		}
